...
public class NutricionistaService {
    ...
    public List<NutricionistaResponseDTO> listarNutricionistas() {
        return nutricionistaRepository.findAll().stream().map(nutricionista -> new NutricionistaResponseDTO(nutricionista.getId(), nutricionista.getNome(), nutricionista.getMatricula(), nutricionista.getTempoExperiencia(), nutricionista.getCrn(), nutricionista.getEspecialidade())).collect(Collectors.toList());
    }
    ...
    public NutricionistaResponseDTO buscarNutricionista(Long id) {
        ...
        if (nutricionista != null) {
            return new NutricionistaResponseDTO(nutricionista.getId(), nutricionista.getNome(), nutricionista.getMatricula(), nutricionista.getTempoExperiencia(), nutricionista.getCrn(), nutricionista.getEspecialidade());
        }
        return null;
    }
    ...
    public NutricionistaResponseDTO salvarNutricionista(NutricionistaRequestDTO request) {
        ...
        return new NutricionistaResponseDTO(entitySalva.getId(), entitySalva.getNome(), entitySalva.getMatricula(), entitySalva.getTempoExperiencia(), entitySalva.getCrn(), entitySalva.getEspecialidade());
    }
    ...
    public NutricionistaResponseDTO atualizarNutricionista(Long id, NutricionistaRequestDTO request) {
        ...
        if (!Objects.equals(nutricionista.getNome(), request.getNome()) && nutricionistaRepository.findByNome(request.getNome()).isPresent()) {
            throw new RuntimeException("Já existe cadastro de nutricionista com este nome.");
        }
