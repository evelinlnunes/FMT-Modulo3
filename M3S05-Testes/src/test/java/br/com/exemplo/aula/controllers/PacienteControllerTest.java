...
class PacienteControllerTest {
    ...
    @BeforeEach
    void setUp() {
        paciente1 = new PacienteResponseDTO(1L, "João Silva", LocalDate.of(1990, 1, 1), "111.111.111-11", "99999999", "joao@email.com");
        paciente2 = new PacienteResponseDTO(2L, "Maria Souza", LocalDate.of(1985, 5, 5), "222.222.222-22", "88888888", "maria@email.com");
    }
        ...
        mvc.perform(post("/pacientes").contentType(MediaType.APPLICATION_JSON).content("""
                {
                    ...
                    "email": "texto",
                    "idEndereco": 1
                }""")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(paciente1.getId())).andExpect(jsonPath("$.nome").value(paciente1.getNome()));
    ...
    void listarPacientes() throws Exception {
        ...
        mvc.perform(get("/pacientes")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(jsonPath("$[0].id").value(paciente1.getId())).andExpect(jsonPath("$[0].nome").value(paciente1.getNome())).andExpect(jsonPath("$[1].id").value(paciente2.getId())).andExpect(jsonPath("$[1].nome").value(paciente2.getNome()));
